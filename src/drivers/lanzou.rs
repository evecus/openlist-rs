//! 蓝奏云驱动（对齐 Go 版 drivers/lanzou，cookie 模式）
//!
//! - 授权：pc.woozooo.com 登录后的 cookie
...
impl Lanzou {
        ...
        match zt {
            ...
            _ => {
                let info = v
                    ...
                    .unwrap_or("unknown")
                    .to_string();
                Err(format!("蓝奏云接口错误(zt={zt}): {info}"))
            }
        }
    }

    /// 对齐 Init()：从 mydisk.php 取 uid / vei
    pub async fn validate(&self) -> Result<(), String> {
        if self.cookie().is_empty() {
            return Err("蓝奏云 cookie 不能为空".into());
        }
    ...
    pub async fn list(&self, folder_id: &str) -> Result<Vec<Entry>, String> {
        let fid = if folder_id.is_empty() { "-1" } else { folder_id };
        // 文件夹
