<template>
  ...
  <VideoPlayer v-if="playing" :file="playing" :src="streamUrl(playing)" @close="closePlayer" />

  <div class="app-shell">
    <header class="app-header">
      <div class="header-inner">
        <div class="brand" @click="view = 'files'">
...
async function loadAccounts() {
  ...
  if (!currentId.value && accounts.value.length > 0) {
    currentId.value = accounts.value[0].id
    onSwitchAccount(currentId.value)
  }
}
...
const streamUrl = (e) => `/api/stream?${fileQuery(e, 'inline')}`

function download(e) {
  window.open(`/api/stream?${fileQuery(e)}`, '_blank')
}
